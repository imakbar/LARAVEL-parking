    ...
    <div>
        <header class="page-header">
            <div class="d-flex align-items-center">
                <div class="mr-auto">
                    <h1 class="separator">Parking Tabs</h1>
                    <nav class="breadcrumb-wrapper" aria-label="breadcrumb">
                        <ol class="breadcrumb">
                            <li class="breadcrumb-item"><a href="/"><i class="icon dripicons-home"></i></a></li>
                            <li class="breadcrumb-item active" aria-current="page">Parking Tabs</li>
                        </ol>
                    </nav>
                </div>
                    ...
                    <form id="myForm">
                        ...
                        <div v-if="record.id != 0 || record.add_form == 1">
                            <div class="row">
                                ...
                                <div class="col-md-4">
                                    <div class="form-group">
                                        <label class="m-top-45"></label>
                                        <label for="open"><input type="checkbox" required="" ref="open" id="open" class=" mr-sm-2" v-model="record.open">Open</label>
                                        <small>
                                            <span v-if="errors.open != null" class="text-danger">
                                                {{errors.open[0]}}
                                            </span>
                                        </small>
                                    </div>
                            ...
                            <div>
                                <button type="submit" id="submit" v-if="record.id == 0 && permissions.includes('setup-parking-tabs-add')" class="btn btn-success btn-rounded btn-floating" @click.prevent="save()">Save</button>
                                <button type="submit" id="submit" v-if="record.id != 0 && permissions.includes('setup-parking-tabs-update')" class="btn btn-primary btn-rounded btn-floating" @click.prevent="update()">Update</button>
                            ...
                            <table class="table-table table table-striped table-bordered thead-inverse dataex-res-configuration bg-white">
                                <thead>
                                    <tr>
                                        <th width="60">S.No</th>
                                        <th>Title</th>
                                        <th width="60" class="text-center">Open</th>
                                ...
                                <draggable class="table-striped" :list="items.data" :element="'tbody'" @change="checkMove">
                                    ...
                                    <tr v-for="(item, index) in items.data" :key="item.id">
                                        ...
                                        <td>{{item.title}}</td>
                                        <td v-if="item.open == 1" class="text-center">
                                            <i class="zmdi zmdi-check"></i>
                                        </td>
                                        <td v-if="item.open != 1" class="text-center"></td>
                                        ...
                                        <td class="text-center">
                                            <button v-if="permissions.includes('setup-parking-tabs-update')" type="button" @click="edit(item.id)" class="btn-pointer btn-warning btn-floating">
                                                <i class="icon-pencil text-size-10 text-white"></i>
                                            </button>
                                            <button v-if="permissions.includes('setup-parking-tabs-delete')" type="button" @click="recordDelete(item.id)" class="btn-pointer btn-danger btn-floating">
...
<script>
    export default {
        ...
        methods: {
            ...
            save() {
                axios.post('/setup/parking-tabs/save', this.record)
                    .then((res) => {
            ...
            getData(page = 1) {
                axios.get('/setup/parking-tabs-get/all/?page='+page)
                    .then((res) => {
                        this.items = res.data
                    })
            },
            searchData(page = 1) {
                axios.post('/setup/parking-tabs/search/?page='+page, this.search)
                    .then((res) => {
                        this.items = res.data
                    })
            },
            edit(id) {
                axios.get('/setup/parking-tabs/find/' + id)
                    .then((res) => {
            ...
            update() {
                axios.post('/setup/parking-tabs/update', this.record)
                    .then((res) => {
                        if (res.data.success == false) {
                            this.errors = res.data.errors
                            this.response = {
            ...
            checkMove(){
                ...
                axios.post('/setup/parking-tabs/sorting', this.items.data)
                    .then((res) => {
                        if (res.data.success == false) {
                            this.errors = res.data.errors
                            this.response = {
            ...
            recordDelete(id) {
                axios.get('/setup/parking-tabs-delete/' + id)
                    .then((res) => {
                        this.response = {
                            isVisible: true,
                            message: res.data.message,
