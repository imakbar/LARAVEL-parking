    ...
    <div>
        <header class="page-header">
            <div class="d-flex align-items-center">
                <div class="mr-auto">
                    <h1 class="separator">Terminals</h1>
                ...
                <div class="actions top-right">
                    ...
                    <a :href="'/setup/airport'" class="btn btn-warning btn-floating btn-rounded text-white">Back</a>
                </div>
            </div>
        </header>
                ...
                <div class="">
                    <form id="myForm">
                        ...
                        <div v-if="record.id != 0 || record.add_form == 1">
                            ...
                            <div>
                                <button type="submit" id="submit" v-if="record.id == 0 && permissions.includes('airport-terminals-add')" class="btn btn-success btn-rounded btn-floating" @click.prevent="save()">Save</button>
                                <button type="submit" id="submit" v-if="record.id != 0 && permissions.includes('airport-terminals-update')" class="btn btn-primary btn-rounded btn-floating" @click.prevent="update()">Update</button>
                                ...
                                <draggable class="table-striped" :list="items.data" :element="'tbody'" @change="checkMove">
                                    ...
                                    <tr v-for="(item, index) in items.data" :key="item.id">
                                        ...
                                        <td class="text-center">
                                            <button v-if="permissions.includes('airport-terminals-update')" type="button" @click="edit(item.id)" class="btn-pointer btn-warning btn-floating">
                                                <i class="icon-pencil text-size-10 text-white"></i>
                                            </button>
                                            <button v-if="permissions.includes('airport-terminals-delete')" type="button" @click="recordDelete(item.id)" class="btn-pointer btn-danger btn-floating">
...
<script>
    export default {
        ...
        methods: {
            ...
            save() {
                axios.post('/setup/airport-terminals/save', this.record)
                    .then((res) => {
            ...
            getData(page = 1) {
                axios.get('/setup/airport-terminals-get/all/'+this.airport.id+'/?page='+page)
                    .then((res) => {
                        this.items = res.data
                    })
            },
            searchData(page = 1) {
                axios.post('/setup/airport-terminals/search/?page='+page, this.search)
                    .then((res) => {
                        this.items = res.data
                    })
            },
            edit(id) {
                axios.get('/setup/airport-terminals/find/' + id)
                    .then((res) => {
                        this.record = res.data
                        this.record.airport_id = this.airport.id
            ...
            update() {
                axios.post('/setup/airport-terminals/update', this.record)
                    .then((res) => {
                        if (res.data.success == false) {
                            this.errors = res.data.errors
                            this.response = {
            ...
            checkMove(){
                ...
                axios.post('/setup/airport-terminals/sorting', this.items.data)
                    .then((res) => {
                        if (res.data.success == false) {
                            this.errors = res.data.errors
                            this.response = {
            ...
            recordDelete(id) {
                axios.get('/setup/airport-terminals-delete/' + id)
                    .then((res) => {
                        this.response = {
                            isVisible: true,
                            message: res.data.message,
