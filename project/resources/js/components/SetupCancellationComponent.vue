    ...
    <div>
        <header class="page-header">
            <div class="d-flex align-items-center">
                <div class="mr-auto">
                    <h1 class="separator">Cancellation</h1>
                ...
                <div class="">
                    <form id="myForm">
                        ...
                        <div v-if="record.id != 0 || record.add_form == 1">
                            ...
                            <div>
                                <button type="submit" id="submit" v-if="record.id == 0 && permissions.includes('setup-cancellation-add')" class="btn btn-success btn-rounded btn-floating" @click.prevent="save()">Save</button>
                                <button type="submit" id="submit" v-if="record.id != 0 && permissions.includes('setup-cancellation-update')" class="btn btn-primary btn-rounded btn-floating" @click.prevent="update()">Update</button>
                                ...
                                <tbody class="table-striped">
                                    <tr v-for="(item, index) in items.data" :key="index">
                                        ...
                                        <td class="text-center">
                                            <button v-if="permissions.includes('setup-cancellation-update')" type="button" @click="edit(item.id)" class="btn-pointer btn-warning btn-floating">
                                                <i class="icon-pencil text-size-10 text-white"></i>
                                            </button>
                                            <button v-if="permissions.includes('setup-cancellation-delete')" type="button" @click="recordDelete(item.id)" class="btn-pointer btn-danger btn-floating">
...
<script>
    export default {
        ...
        methods: {
            ...
            save() {
                axios.post('/setup/cancellation/save', this.record)
                    .then((res) => {
            ...
            getData(page = 1) {
                axios.get('/setup/cancellation-get/all/?page='+page)
                    .then((res) => {
                        this.items = res.data
                    })
            },
            searchData(page = 1) {
                axios.post('/setup/cancellation/search/?page='+page, this.search)
                    .then((res) => {
                        this.items = res.data
                    })
            },
            edit(id) {
                axios.get('/setup/cancellation/find/' + id)
                    .then((res) => {
            ...
            update() {
                axios.post('/setup/cancellation/update', this.record)
                    .then((res) => {
                        if (res.data.success == false) {
                            this.errors = res.data.errors
                            this.response = {
            ...
            recordDelete(id) {
                axios.get('/setup/cancellation-delete/' + id)
                    .then((res) => {
                        this.response = {
                            isVisible: true,
                            message: res.data.message,
