    ...
    <div>
        <header class="page-header">
            <div class="d-flex align-items-center">
                <div class="mr-auto">
                    <h1 class="separator">Blog Post</h1>
                        ...
                        <div v-if="record.id != 0 || record.add_form == 1">
                            <div class="row">
                                <div class="col-md-3 m-bottom-20 text-center" v-if="record.id != 0">
                                    <img :src="'/uploads/posts/'+record.avatar" alt='Image preview' class='img-thumbnail card-img-top' v-if="record.avatar != null">
                                                ...
                                                <div class="modal-body">
                                                    ...
                                                    <div class="img-upload">
                                                        <div class="img-file-tab">
                                                            ...
                                                            <img :src="'/uploads/posts/'+record.avatar" alt='Image preview' class='img-thumbnail w-100' v-if="record.avatar != null">
                                                        </div>
                                                    </div>
                            ...
                            <div class="row">
                                <div class="col-md-4">
                                    <div class="form-group tooltip_error">
                                        <label for="blogcategorysetup_id">Category</label>
                                        <select v-model="record.blogcategorysetup_id" name="blogcategorysetup_id" id="blogcategorysetup_id" class="form-control">
                                            <option value="">-- select --</option>
                                            <option v-for="blog_category_setup in blog_category_setups" :value="blog_category_setup.id" :key="blog_category_setup.id">{{blog_category_setup.title}}</option>
                                        </select>
                                        <small v-if="errors.blogcategorysetup_id != null" class="text-danger text-size-11">
                                            <span>
                                                {{errors.blogcategorysetup_id[0]}}
                                            </span>
                                        </small>
                                    </div>
                                </div>
                            ...
                            <div class="row">
                                <div class="col-md-4">
                                    <div class="form-group">
                                        ...
                                        <input type="text" required="" ref="meta_tags" id="meta_tags" class="form-control mr-sm-2"
                                ...
                                <div class="col-md-4">
                                    <div class="form-group">
                                        ...
                                        <input type="text" required="" ref="meta_description" id="meta_description" class="form-control mr-sm-2"
                            ...
                            <div>
                                <button type="submit" id="submit" v-if="record.id == 0 && permissions.includes('blog-post-add')" class="btn btn-success btn-rounded btn-floating" @click.prevent="save()">Save</button>
                                <button type="submit" id="submit" v-if="record.id != 0 && permissions.includes('blog-post-update')" class="btn btn-primary btn-rounded btn-floating" @click.prevent="update()">Update</button>
                            ...
                            <table class="table-table table table-striped table-bordered thead-inverse dataex-res-configuration bg-white">
                                ...
                                <tbody class="table-striped">
                                    <tr v-for="(item, index) in items.data" :key="index">
                                        <td>{{index + 1}}</td>
                                        <td width="120">{{item.blog_category_setup.title}}</td>
                                        <td width="70"><img :src="'/uploads/posts/'+item.avatar" alt='Image preview' class='img-thumbnail card-img-top' v-if="item.avatar != null"></td>
                                        <td>{{item.title}}</td>
                                        ...
                                        <td width="90" class="text-center">
                                            <button v-if="permissions.includes('blog-post-update')" type="button" @click="edit(item.id)" class="btn-pointer btn-warning btn-floating">
                                                <i class="icon-pencil text-size-10 text-white"></i>
                                            </button>
                                            <button v-if="permissions.includes('blog-post-delete')" type="button" @click="recordDelete(item.id)" class="btn-pointer btn-danger btn-floating">
    ...
    export default {
        data() {
            return {
                ...
                errors: [],
                permissions: [],
                blog_category_setups: [],
            }
        },
        created() {
            this.getPermissions()
            this.getData()
            this.getBlogCategorySetups()
        },
        ...
        methods: {
            ...
            save() {
                axios.post('/blog-post/save', this.record)
                    .then((res) => {
            ...
            getData(page = 1) {
                axios.get('/blog-post-get/all/?page='+page)
                    .then((res) => {
                        this.items = res.data
                    })
            },
            searchData(page = 1) {
                axios.post('/blog-post/search/?page='+page, this.search)
                    .then((res) => {
                        this.items = res.data
                    })
            },
            edit(id) {
                axios.get('/blog-post/find/' + id)
                    .then((res) => {
            ...
            update() {
                axios.post('/blog-post/update', this.record)
                    .then((res) => {
                        if (res.data.success == false) {
                            this.errors = res.data.errors
                            this.response = {
            ...
            updateAvatar() {
                ...
                axios.post('/blog-post/update/avatar', data)
                    .then((res) => {
                        if (res.data.success == false) {
                            this.errors = res.data.errors
                            this.response = {
