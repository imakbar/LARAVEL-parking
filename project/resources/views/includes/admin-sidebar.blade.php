	...
	<div class="sidebar-content">
			...
			<ul class="site-logo">
				...
				<!-- START LOGO -->
					<a href="{{url('/')}}/dashboard">
						...
						<span class="brand-text">{{$ASPTitle}}</span>
					</a>
				<!-- END LOGO -->
				</li>
			</ul>
		...
		<nav class="main-menu">
			<ul class="nav metismenu">
				...
				@foreach(config('acl.modules') as $module)
	            @php
	                $hasItems = false;
	            @endphp
	            @foreach($module['forms'] as $form)
	                @if(ACL::hasRole($form['permission']))
	                    @php
	                        $hasItems = true;
	                    @endphp
	                @endif
	            @endforeach
	            @if($hasItems || $module['url'] != null)
		            <li class="nav-dropdown {{ (Request::is($form['url']) ? 'active' : '') }}">
		                @if(count($module['forms']) != 0)
			                <a href="{{$module['url']}}" class="has-arrow" aria-expanded="false">
			                ...
			                <ul class="collapse nav-sub" aria-expanded="false">
			                    @foreach($module['forms'] as $form)
			                        @if(ACL::hasRole($form['permission']))
				                        <li class="{{ (Request::is($form['url']) ? 'active' : '') }}">
				                            <a href="{{url('/')}}/{{$form['url']}}">
				                                <span>{{$form['name']}}</span>
				                            </a>
				                        </li>
