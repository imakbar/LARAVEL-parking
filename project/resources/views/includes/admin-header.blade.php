...
<nav class="top-toolbar navbar navbar-desktop flex-nowrap">
	<ul class="navbar-nav nav-left">
		<li class="nav-item nav-text dropdown-menu-md">
	...
	<ul class="navbar-nav nav-right">
		<li class="nav-item dropdown">
			<a class="nav-link nav-pill user-avatar" data-toggle="dropdown" href="#" role="button" aria-haspopup="true" aria-expanded="false">
				<img src="{{url('/')}}/uploads/profiles/sm-{{$ASPProfile->avatar}}" class="w-35 rounded-circle" alt="{{$ASPProfile->first_name}} {{$ASPProfile->last_name}}">
			</a>
			<div class="dropdown-menu dropdown-menu-right dropdown-menu-accout">
				<div class="dropdown-header pb-3">
					<div class="media d-user">
						<img class="align-self-center mr-3 w-40 rounded-circle" src="{{url('/')}}/uploads/profiles/sm-{{$ASPProfile->avatar}}" alt="{{$ASPProfile->first_name}} {{$ASPProfile->last_name}}">
						<div class="media-body">
							<h5 class="mt-0 mb-0">{{$ASPProfile->first_name}} {{$ASPProfile->last_name}}</h5>
							<span>{{$ASPUserEmail}}</span>
						</div>
					</div>
				</div>
				<a class="dropdown-item" href="{{url('/')}}/profile"><i class="icon dripicons-user"></i> Profile</a>
				@if(Auth::user()->user_type == 'super_admin')
				<a class="dropdown-item" href="{{url('/')}}/settings"><i class="icon dripicons-user"></i> Settings</a>
				@endif
				<a class="dropdown-item" href="{{url('/')}}/change-password"><i class="icon dripicons-lock"></i> Change Password</a>
				<a class="dropdown-item" href="{{url('/')}}/logout"><i class="icon dripicons-lock-open"></i> Sign Out</a>
			</div>
		</li>
	</ul>
