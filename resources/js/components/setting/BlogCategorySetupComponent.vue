    ...
    <div>
        <header class="page-header">
            <div class="d-flex align-items-center">
                <div class="mr-auto">
                    <h1 class="separator">Blog Category</h1>
                ...
                <div class="">
                    <form id="myForm">
                        ...
                        <div v-if="record.id != 0 || record.add_form == 1">
                            ...
                            <div>
                                <button type="submit" id="submit" v-if="record.id == 0 && permissions.includes('blog-category-setup-add')" class="btn btn-success btn-rounded btn-floating" @click.prevent="save()">Save</button>
                                <button type="submit" id="submit" v-if="record.id != 0 && permissions.includes('blog-category-setup-update')" class="btn btn-primary btn-rounded btn-floating" @click.prevent="update()">Update</button>
                                ...
                                <tbody class="table-striped">
                                    <tr v-for="(item, index) in items.data" :key="index">
                                        ...
                                        <td class="text-center">
                                            <button v-if="permissions.includes('blog-category-setup-update')" type="button" @click="edit(item.id)" class="btn-pointer btn-warning btn-floating">
                                                <i class="icon-pencil text-size-10 text-white"></i>
                                            </button>
                                            <button v-if="permissions.includes('blog-category-setup-delete')" type="button" @click="recordDelete(item.id)" class="btn-pointer btn-danger btn-floating">
...
<script>
    export default {
        ...
        methods: {
            ...
            save() {
                axios.post('/blog-category-setup/save', this.record)
                    .then((res) => {
            ...
            getData(page = 1) {
                axios.get('/blog-category-setup-get/all/?page='+page)
                    .then((res) => {
                        this.items = res.data
                    })
            },
            searchData(page = 1) {
                axios.post('/blog-category-setup/search/?page='+page, this.search)
                    .then((res) => {
                        this.items = res.data
                    })
            },
            edit(id) {
                axios.get('/blog-category-setup/find/' + id)
                    .then((res) => {
            ...
            update() {
                axios.post('/blog-category-setup/update', this.record)
                    .then((res) => {
                        if (res.data.success == false) {
                            this.errors = res.data.errors
                            this.response = {
            ...
            recordDelete(id) {
                axios.get('/blog-category-setup-delete/' + id)
                    .then((res) => {
                        this.response = {
                            isVisible: true,
                            message: res.data.message,
