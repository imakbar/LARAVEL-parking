    ...
    <div>
        <header class="page-header">
            <div class="d-flex align-items-center">
                <div class="mr-auto">
                    <h1 class="separator">Parking Services</h1>
                    <nav class="breadcrumb-wrapper" aria-label="breadcrumb">
                        <ol class="breadcrumb">
                            <li class="breadcrumb-item"><a href="/"><i class="icon dripicons-home"></i></a></li>
                            <li class="breadcrumb-item active" aria-current="page">Parking Services</li>
                        </ol>
                    </nav>
                </div>
                        ...
                        <div v-if="record.id != 0 || record.add_form == 1">
                            <div class="row">
                                <div class="col-md-3 m-bottom-20 text-center" v-if="record.id != 0">
                                    <img :src="'/uploads/parking-services/'+record.avatar" :alt='record.avatar' class='img-thumbnail card-img-top' v-if="record.avatar">
                                                ...
                                                <div class="modal-body">
                                                    ...
                                                    <div class="img-upload">
                                                        <div class="img-file-tab">
                                                            ...
                                                            <img :src="'/uploads/parking-services/'+record.avatar" :alt='record.avatar' class='img-thumbnail w-100' v-if="record.avatar">
                                                        </div>
                                                    </div>
                            ...
                            <div>
                                <button type="submit" id="submit" v-if="record.id == 0 && permissions.includes('setup-parking-services-add')" class="btn btn-success btn-rounded btn-floating" @click.prevent="save()">Save</button>
                                <button type="submit" id="submit" v-if="record.id != 0 && permissions.includes('setup-parking-services-update')" class="btn btn-primary btn-rounded btn-floating" @click.prevent="update()">Update</button>
                    ...
                    <div class="row" v-if="record.id == 0 && record.add_form == 0">
                        <div class="col-md-12 table-responsive">
                            <table class="table-table table table-striped table-bordered thead-inverse dataex-res-configuration bg-white">
                                ...
                                <draggable class="table-striped" :list="items.data" :element="'tbody'" @change="checkMove">
                                    <tr v-for="(item, index) in items.data" :key="index">
                                        <td>{{index + 1}}</td>
                                        <td width="70"><img :src="'/uploads/parking-services/'+item.avatar" alt='Image preview' class='img-thumbnail card-img-top' v-if="item.avatar != null"></td>
                                        <td>{{item.title}}</td>
                                        ...
                                        <td width="90" class="text-center">
                                            <button v-if="permissions.includes('setup-parking-services-update')" type="button" @click="edit(item.id)" class="btn-pointer btn-warning btn-floating">
                                                <i class="icon-pencil text-size-10 text-white"></i>
                                            </button>
                                            <button v-if="permissions.includes('setup-parking-services-delete')" type="button" @click="recordDelete(item.id)" class="btn-pointer btn-danger btn-floating">
...
<script>
    export default {
        ...
        methods: {
            ...
            save() {
                axios.post('/setup/parking-services/save', this.record)
                    .then((res) => {
            ...
            getData(page = 1) {
                axios.get('/setup/parking-services-get/all/?page='+page)
                    .then((res) => {
                        this.items = res.data
                    })
            },
            searchData(page = 1) {
                axios.post('/setup/parking-services/search/?page='+page, this.search)
                    .then((res) => {
                        this.items = res.data
                    })
            },
            edit(id) {
                axios.get('/setup/parking-services/find/' + id)
                    .then((res) => {
            ...
            update() {
                axios.post('/setup/parking-services/update', this.record)
                    .then((res) => {
                        if (res.data.success == false) {
                            this.errors = res.data.errors
                            this.response = {
            ...
            updateAvatar() {
                ...
                axios.post('/setup/parking-services/update/avatar', data)
                    .then((res) => {
                        if (res.data.success == false) {
                            this.errors = res.data.errors
                            this.response = {
            ...
            checkMove(){
                ...
                axios.post('/setup/parking-services/sorting', this.items.data)
                    .then((res) => {
                        if (res.data.success == false) {
                            this.errors = res.data.errors
                            this.response = {
            ...
            recordDelete(id) {
                axios.get('/setup/parking-services-delete/' + id)
                    .then((res) => {
                        this.response = {
                            isVisible: true,
                            message: res.data.message,
