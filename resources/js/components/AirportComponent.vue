                ...
                <div class="">
                    <form id="myForm">
                        ...
                        <div v-if="record.id != 0 || record.add_form == 1">
                            <div class="row">
                                <div class="col-md-2">
                                    <div class="form-group">
                                        ...
                                        <input type="text" required="" ref="short_name" id="short_name" class="form-control mr-sm-2"
                                            v-model="record.short_name">
                                        <small>
                                            <span v-if="errors.short_name != null" class="text-danger">
                                                {{errors.short_name[0]}}
                                            </span>
                                        </small>
                                    </div>
                                ...
                                <div class="col-md-4">
                                    <div class="form-group">
                                        ...
                                        <input type="text" required="" ref="name" id="name" class="form-control mr-sm-2"
                                            v-model="record.name">
                                        <small>
                            ...
                            <div>
                                <button type="submit" id="submit" v-if="record.id == 0 && permissions.includes('airport-add')" class="btn btn-success btn-rounded btn-floating" @click.prevent="save()">Save</button>
                                <button type="submit" id="submit" v-if="record.id != 0 && permissions.includes('airport-update')" class="btn btn-primary btn-rounded btn-floating" @click.prevent="update()">Update</button>
                            ...
                            <table class="table-table table table-striped table-bordered thead-inverse dataex-res-configuration bg-white">
                                ...
                                <tbody class="table-striped">
                                    <tr v-for="(item, index) in items.data" :key="index">
                                        <td>{{index + 1}}</td>
                                        <td><a :href="'/setup/airport-terminals/'+item.id">Terminal ({{item.terminals.length}})</a></td>
                                        <td width="150">{{item.short_name}}</td>
                                        <td>{{item.name}}</td>
                                        ...
                                        <td class="text-center">
                                            <button v-if="permissions.includes('airport-update')" type="button" @click="edit(item.id)" class="btn-pointer btn-warning btn-floating">
                                                <i class="icon-pencil text-size-10 text-white"></i>
                                            </button>
                                            <button v-if="permissions.includes('airport-delete')" type="button" @click="recordDelete(item.id)" class="btn-pointer btn-danger btn-floating">
...
<script>
    export default {
        ...
        methods: {
            ...
            save() {
                axios.post('/setup/airport/save', this.record)
                    .then((res) => {
            ...
            getData(page = 1) {
                axios.get('/setup/airport-get/all/?page='+page)
                    .then((res) => {
                        this.items = res.data
                    })
            },
            searchData(page = 1) {
                axios.post('/setup/airport/search/?page='+page, this.search)
                    .then((res) => {
                        this.items = res.data
                    })
            },
            edit(id) {
                axios.get('/setup/airport/find/' + id)
                    .then((res) => {
            ...
            update() {
                axios.post('/setup/airport/update', this.record)
                    .then((res) => {
                        if (res.data.success == false) {
                            this.errors = res.data.errors
                            this.response = {
            ...
            recordDelete(id) {
                axios.get('/setup/airport-delete/' + id)
                    .then((res) => {
                        this.response = {
                            isVisible: true,
                            message: res.data.message,
